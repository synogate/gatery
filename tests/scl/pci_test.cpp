...
BOOST_FIXTURE_TEST_CASE(tlp_serializer_test, BoostUnitTestSimulationFixture)
{
	Clock clk = Clock({ .absoluteFrequency = 100'000'000 });
	ClockScope clkScope(clk);

	scl::pci::TlpIntelPacketStream<> in;
	pinIn(in, "in");

	//scl::pci::TlpPacketStream<> out = scl::pci::toTlpPacketStream(move(in));
	//pinOut(out, "out");
	...
	//design.visualize("tlp_serializer_test");
}
