...
BOOST_FIXTURE_TEST_CASE(FifoArray_poc, BoostUnitTestSimulationFixture) {
	...
	size_t ElementsPerFifo = 4;
	BVec dataSample = 4_b;
	scl::FifoArray<BVec> dut_fifo{numberOfFifos, ElementsPerFifo, dataSample};

	BVec push_data = constructFrom(dataSample);
	pinIn(push_data, "push_data");
	dut_fifo.present(push_data);

	Bit push_enable;
	pinIn(push_enable, "push_enable");

	UInt pushFifoSelector = BitWidth::count(numberOfFifos);
	pinIn(pushFifoSelector, "pushFifoSelector");

	IF(push_enable) {
		dut_fifo.push(pushFifoSelector);
	}

	pinOut(dut_fifo.peek(), "pop_data");

	Bit pop_enable;
	pinIn(pop_enable, "pop_enable");

	UInt popFifoSelector = BitWidth::count(numberOfFifos);
	pinIn(popFifoSelector, "popFifoSelector");

	IF(pop_enable) {
		dut_fifo.pop(popFifoSelector);
	}

	pinOut(dut_fifo.pop_failed(), "pop_failed");
	pinOut(dut_fifo.push_failed(), "push_failed");

	addSimulationProcess([&, this]()->SimProcess {
		simu(push_data) = 1;
		simu(push_enable) = '1';
		simu(pop_enable) = '0';
		simu(pushFifoSelector) = 0;
		
		co_await OnClk(clk);

		BOOST_TEST(simu(dut_fifo.push_failed()) == '0');
		...
		simu(pop_enable) = '1';
		simu(popFifoSelector) = 0;

		co_await OnClk(clk);

		simu(pop_enable) = '0';

		BOOST_TEST(simu(dut_fifo.pop_failed()) == '0');
		BOOST_TEST(simu(dut_fifo.peek()) == 1);
...
BOOST_FIXTURE_TEST_CASE(FifoArray_fillTest, BoostUnitTestSimulationFixture) {
	Clock clk({ .absoluteFrequency = 100'000'000 });
	ClockScope clkScp(clk);
	...
	addSimulationProcess([&, this]()->SimProcess {
		...
		for (size_t i = 0; i < ElementsPerFifo; i++)
		{
			co_await OnClk(clk);
			BOOST_TEST(simu(dut_fifo.push_failed()) == '0');
		}
		...
		co_await OnClk(clk);
		BOOST_TEST(simu(dut_fifo.push_failed()) == '1');
...
BOOST_FIXTURE_TEST_CASE(FifoArray_emptyTest, BoostUnitTestSimulationFixture) {
	Clock clk({ .absoluteFrequency = 100'000'000 });
	ClockScope clkScp(clk);
	...
	addSimulationProcess([&, this]()->SimProcess {
		...
		co_await OnClk(clk);
		BOOST_TEST(simu(dut_fifo.pop_failed()) == '1');
...
BOOST_FIXTURE_TEST_CASE(FifoArray_pushpop_same_fifo, BoostUnitTestSimulationFixture) {
	Clock clk({ .absoluteFrequency = 100'000'000 });
	ClockScope clkScp(clk);
...
BOOST_FIXTURE_TEST_CASE(FifoArray_fuzz, BoostUnitTestSimulationFixture) {
	Clock clk({ .absoluteFrequency = 100'000'000 });
	ClockScope clkScp(clk);
	...
	std::uniform_int_distribution<size_t> fifo_rng(0, numberOfFifos-1);
	std::uniform_int_distribution<size_t> element_rng(0, elementsPerFifo-1);
	std::uniform_int_distribution<size_t> wait_rng(0, 5);

	std::vector<std::queue<uint8_t>> twin(numberOfFifos);
	...
	/* push things into fifos*/
	addSimulationProcess([&, this]()->SimProcess {
		for (size_t i = 0; i < 100; i++)
		{
			size_t numFifo = fifo_rng(gen) & (numberOfFifos-1);
			simu(pushFifoSelector) = numFifo;

			size_t element = element_rng(gen) & dataSample.width().last();
			simu(push_data) = element;
			...
			if (simu(dut_fifo.push_failed()) == '0')
				twin[numFifo].push(element);

			size_t wait_cycles = wait_rng(gen);
			for (size_t i = 0; i < wait_cycles; i++)
				co_await OnClk(clk);
	...
	addSimulationProcess([&, this]()->SimProcess {
		while (!test_finished) 
		{
			...
			if (simu(dut_fifo.pop_failed()) == '0') {
				BOOST_TEST(twin[numFifo].front() == simu(dut_fifo.peek()));
				twin[numFifo].pop();
			}
	...
	design.postprocess();
	BOOST_TEST(!runHitsTimeout({ 200, 1'000'000 }));
}
