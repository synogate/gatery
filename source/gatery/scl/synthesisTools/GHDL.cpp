...
void GHDL::writeStandAloneProject(vhdl::VHDLExport& vhdlExport, std::string_view filename)
{
	...
	for (const auto &e : vhdlExport.getTestbenchRecorder()) {
		...
		file << "ghdl -r --std=08 -frelaxed " << library << e->getDependencySortedEntities().back() << " --ieee-asserts=disable --vcd=" << e->getName() << "_signals.vcd --wave=" << e->getName() << "_signals.ghw" << std::endl;
	}
}
